...
public class WordCloudData {
	...
	public void WordCloudData(String inputString) {
		populateWordsToCounts(inputString);
	}

	public void populateWordsToCounts(String inputString) {
		...
		for (int i = 0; i < inputString.length(); i++) {
			...
			else if (i > 0 && Character.isLetter(inputString.charAt(i - 1))
	...
	public void addWordToHashMap(String word) {
		...
		if (wordsToCounts.containsKey(word)) {
			...
			// if a lowercase version is in the hash map, we know our input word
			// must be uppercase
			...
			// if an uppercase version is in the hash map, we know our input
			// word must be lowercase.
			// since we only include uppercase words if they're always
			// uppercase, we add the
			...
			// otherwise, the word is not in the hash map at all, lowercase or
			// uppercase
